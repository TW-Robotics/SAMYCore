...
namespace SAMY{
    ...
    void SAMYSkill::setTransitions( UA_Server* server ){
         transitions = generateTransitions( server );
    }
    void SAMYSkill::setStates( UA_Server* server ){
         states = generateStates( server );
    }
    void SAMYSkill::setInitialStateSkill(){
    ...
    //        LockedServer ls = server->getLocked();
      ...
      //          logger->warn("createEvent failed. StatusCode {}", UA_StatusCode_name(retval));
                std::cout << " ERROR GENERATING EVENT triggerTransitionEvent" << std::endl;
                return retval;
            }
        }
        ...
        UA_StatusCode retval = UA_Server_triggerEvent(server, eventId, skillNodeID, nullptr, UA_TRUE);
            if (retval != UA_STATUSCODE_GOOD) {
                std::cout << " ERROR TRIGGERING EVENT triggerTransitionEvent" << std::endl;
                return retval;
            }
