...
(define (domain samy-min-temporal)
...
(:functions
 (move-duration ?r - robot ?startpose ?endpose - pose)
)
...
(:durative-action move-to-pickup
 ...
 :condition (and
  ...
  (at start (is-at ?res ?oldrobotpose)) ; the robot starts at its old pose
 ...
 :effect (and
  (at end (is-at ?res ?robotpose)) ; the robot is now in robotpose
  (at start (not (is-at ?res ?oldrobotpose))) ; the robot is no longer in the oldrobotpose
...
;  (at start (is-at ?res ?robotpose)) ; the robot is at robotpose
...
(:durative-action wait-for-gripper-closed-to-pickup
 ...
 :effect(and
  ...
  (at end (is-at ?obj ?res)) ; the robot is now holding the object
  (at start (not (idle ?res))) ; the robot cannot do other actions in the meantime
  (at end (idle ?res)) ; the robot can do other actions again
 )
)
...
(:durative-action move-from-pickup-to-place
 ...
 :condition(and
  ...
  (over all (is-at ?obj ?res)) ; the robot is holding the object
  (at start (is-at ?res ?oldrobotpose)) ; the robot is in the oldrobotpose
