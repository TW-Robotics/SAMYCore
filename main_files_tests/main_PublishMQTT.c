...
static void addSAMYRobotNode( UA_Server* server ){
    ...
    pubSubAddresses.iPAddress_Skill = UA_STRING("opc.mqtt://127.0.0.1:1883");
    pubSubAddresses.iPAddress_Status = UA_STRING("opc.mqtt://127.0.0.1:1883");
...
int main(int argc, char** argv) {
   ...
   UA_NodeId nodeToPublish = UA_NODEID_STRING(1, "SAMYRobot");
    ...
    if(namespace_crcl_opcua_generated(server) != UA_STATUSCODE_GOOD) {
        ...
        configureSAMYRobotPublisherMQTT_WithoutLogin(&robot, 100, 10, UA_FALSE);
