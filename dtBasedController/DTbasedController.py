...
# I didn't know the format for every combination of possible labels/predicates/single input/multiinput/determined/undetermined actions/etc. in advance because there is no clear documentation that explains the dot format generated for all possible combinations.
# Hende I had to kind of discover it on the run, so the code could be improved (specially the parts regarding the handle of labels)
# The code regarding the inspection of the graph and generation of nodes I think cannot be improved a lot, since we do not pass metadata in advance and they are infered on the run, we need to go through it at least twice to update some node elements that refer to metadata of the whole graph (for example linear split coefficients) 
...
class DTbasedController:
    ...
    def save_controller_metadata(self):
        modified_name = self.path + '_X_metadata'
        ...
        modified_name = self.path + '_Y_metadata'
        with open( modified_name, 'w+') as outfile:
    ...
    def parse_dot(self):
        ...
        self.graph = graph
        rootNode = None     
        for parsedNode in graph.nodes: # We find the root node and ensure that all the leaf nodes labels are correct
        ...
        self.updateLinearSplits( self.root ) # updates the splits taking into account all the variables appearing
         # UPDATE CategoricalMultiSplits Value Groups!
