/* C++17 does not support string views as non-template parameters (as C++20 does). In order to avoid requiring a C++20 compiler, I use this
 Basically given a char* at compile time it can use that char* as non-type parameter of a template, so we can use char* to invoke certain instantiation of a template (this way we can use a typename for example to cast to a certain type)
Everything happens at compile time, so should not have relevant impact on performance at runtime
*/

// Based on: https://egori.ch/2020/10/08/cpp-c-literals-as-template-args.html
